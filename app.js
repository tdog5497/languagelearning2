...
class DataStore {
  ...
  init() {
    ...
    if (user) {
      ...
      // Auto-create a default user for no-login mode
      this.currentUser = {
        id: 'default-user',
        email: 'user@danishdeck.app',
...
function App() {
  const [isAuthenticated, setIsAuthenticated] = useState(true); // Changed to true for no-login mode
  const [currentView, setCurrentView] = useState('dashboard');
  const [refreshKey, setRefreshKey] = useState(0);

  useEffect(() => {
    const user = db.getCurrentUser();
    setIsAuthenticated(true); // Always authenticated in no-login mode
  }, []);
